...
# readonly SCRIPT_NS="bitrab_proc"
readonly SRC_DIR="./bitrab"
readonly TMP_ROOT="/tmp"
readonly TMP_DIR="${TMP_ROOT}/bitrab"
readonly OUTPUT_MD_BASENAME="bitrab.flat.md"

# Copy directory safely
bitrab_proc::copy_to_tmp() {
  if [[ ! -d "${SRC_DIR}" ]]; then
...
# Run strip-docs for each subdirectory of tmp/bitrab
bitrab_proc::run_strip_docs() {
  local dir
...
# in the generated Markdown (bitrab.flat.md). Other code fences remain untouched.
bitrab_proc::clean_flat_md_trailing_double_hash() {
  local output_md
...
main() {
  bitrab_proc::copy_to_tmp
  bitrab_proc::run_strip_docs
  bitrab_proc::confirm "Run coderoller-flatten-repo?"
  bitrab_proc::flatten_repo
  # Post-process the flattened Markdown to remove trailing '##' in Python code blocks only
  bitrab_proc::clean_flat_md_trailing_double_hash
  bitrab_proc::confirm "Delete temporary files in '${TMP_DIR}'?"
  bitrab_proc::cleanup_tmp
}
